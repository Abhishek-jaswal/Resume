...
export default function Education() {
  return (
    <section id="education" className="relative py-28 px-6">
      {/* Ambient */}
      <div className="absolute bottom-0 right-1/3 w-[400px] h-[250px] bg-green-500/[0.04] blur-[80px] rounded-full pointer-events-none" />
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        >
          {education.map((edu) => (
            <motion.div key={edu.title} variants={item}>
              <div className="group relative h-full bg-[#0a0d18]/80 backdrop-blur rounded-2xl p-6 border border-white/[0.06] hover:border-green-500/20 transition-all duration-400 hover:shadow-xl hover:shadow-green-500/[0.06] text-center">
                {/* Rank indicator */}
                <div className="absolute top-4 right-4 w-7 h-7 rounded-full bg-white/[0.04] border border-white/[0.07] flex items-center justify-center text-[11px] text-gray-600 font-mono">
                  0{edu.rank}
                </div>
                ...
                {/* Title */}
                <h3 className="font-syne text-base font-bold text-white mb-1 leading-snug">{edu.title}</h3>
                <p className="text-green-400 text-sm font-medium mb-2">{edu.institution}</p>
